...
on the screen, using asterisk (*) characters. Trailing spaces should be r
emoved, and every line must be terminated with a newline character (\n).
...
def diamond(n):
    ...
    for i in range((n + 1) // 2):
        num_stars = 2 * i + 1
        padding = " " * (n // 2 - i)
        stars.append(padding + "*" * num_stars)
    return "\n".join(stars + stars[-2::-1]) + "\n"