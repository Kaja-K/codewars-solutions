...
m for minutes, s for seconds) are positive or null integer (represented as strings) 
...
def stat(strg):
    if not strg:
        return ""
    s = sorted(int(t[0])*3600 + int(t[1])*60 + int(t[2])
               for t in (x.split("|") for x in strg.split(", ")))
    n = len(s)
    fmt = lambda v: f"{int(v)//3600:02}|{int(v)%3600//60:02}|{int(v)%60:02}"
    med = s[n//2] if n%2 else (s[n//2-1]+s[n//2])/2
    return f"Range: {fmt(s[-1]-s[0])} Average: {fmt(sum(s)/n)} Median: {fmt(med)}"